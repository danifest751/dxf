export function parseDXFMainThread(content){
  ...
  function endPts(ent){
    if(!ent) return [];
    if(ent.type==='LINE') return [{x:ent.raw.x1,y:ent.raw.y1},{x:ent.raw.x2,y:ent.raw.y2}];
    if(ent.type==='ARC'){
      const cx=ent.raw.cx||0, cy=ent.raw.cy||0, r=ent.raw.r||0;
      let A1=(ent.raw.a1||0)*Math.PI/180, A2=(ent.raw.a2||0)*Math.PI/180; if(A2<A1) A2+=2*Math.PI;
    ...
    if(ent.type==='POLY'){
      ...
      if(ent.raw.closed) return []; // closed contour -> independent loop
  ...
  function candPts(ent){
    if(ent.type==='CIRCLE'){
      const {cx=0,cy=0,r=0}=ent.raw||{};
      return [{x:cx+r,y:cy}];
    }
    return endPts(ent);
  }
  function pickPierce(cands){
    if(!cands.length) return null;
    cands.sort((a,b)=> a.x===b.x ? a.y-b.y : a.x-b.x);
    const c=cands[0]; return [c.x,c.y];
  }

  
  // === Pierce detection (quantized endpoint adjacency) ===
  const EPS = (typeof window!=='undefined' && typeof window.PIERCE_EPS==='number') ? window.PIERCE_EPS : 0.8; // mm quantization
